...
if __name__ == "__main__":
    ...
    height_tensor = one_train_data[1]["height_tensor"]
    width_tensor = one_train_data[1]["width_tensor"]
    # original_image_size = (224,224)



    original_image_size_tensor = tf.stack([height_tensor, width_tensor], -1).numpy()
    input_size = (224)

    # custom_model.load_weights(config.model_path)

    # model_output = custom_model(one_train_data[0]["x"]).numpy()
    # model_output = custom_model(one_train_data[0]["x"])
    pred_bbox = custom_model.predict(one_train_data[0]["x"]) # 3, (32, 28, 28, 3, 85) 
    ...
    for i in range(len(pred_bbox[0])):
        ...
        one_pred_bbox = tf.concat(one_pred_bbox, axis=0)
        one_original_image_size = original_image_size_tensor[i]
        ...
        original_image = tf.image.resize(one_train_data[0]["x"][i]*255, original_image_size_tensor[i]).numpy()

        print("original_image:", original_image.shape)



        image = draw_bbox(original_image, bboxes, config.cls_label)
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        cv2.imwrite("model_output_result_{}.png".format(i),image)
    ...
    # small_true_boxes = one_train_data[1]["small_true_boxes"]
    # middle_true_boxes = one_train_data[1]["middle_true_boxes"]
    # large_true_boxes = one_train_data[1]["large_true_boxes"]

    # true_boxes = [small_true_boxes, middle_true_boxes,large_true_boxes]

    # box_list = []
    # for i in range(len(model_output)): 
    
    #     outputRescaler = OutputRescaler(config.anchors[i])
