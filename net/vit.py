...
class Mlp(tf.keras.layers.Layer):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0., prefix=''):
        ...
        self.fc1 =tf.keras.layers.Dense(hidden_features, name=f'{prefix}/mlp/fc1')
        self.fc2 =tf.keras.layers.Dense(out_features, name=f'{prefix}/mlp/fc2')
...
def window_partition(x, window_size):
    ...
    # since there's torch permute, it will becomes [batch, H // window_size,H // window_size,window_size, window_size, C] 
...
def window_reverse(windows, window_size, H, W, C):
    ...
    x = tf.transpose(x, perm=[0, 1, 3, 2, 4, 5])
    x = tf.reshape(x, shape=[-1, H, W, C]) # TensorShape([1, 5, 3, 5, 3, 20]) => TensorShape([1, 15, 15, 20])
    return x
...
class WindowAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #                                                     initializer=tf.initializers.Zeros(), trainable=True) # TensorShape([25, 5])

        self.relative_position_bias_table = self.add_weight(f'{self.prefix}/attn/relative_position_bias_table',
                                                            shape=(
                                                                (2 * self.window_size[0] - 1) * (2 * self.window_size[1] - 1), self.num_heads),
                                                            initializer=tf.keras.initializers.TruncatedNormal(), trainable=True) # TensorShape([25, 5])
        ...
        coords = np.stack(np.meshgrid(coords_h, coords_w, indexing='ij')) # 2 * (3,3) => (2,3,3) 
        coords_flatten = coords.reshape(2, -1) # (2,9)
        relative_coords = coords_flatten[:, :,
                                         None] - coords_flatten[:, None, :]
        '''
        for x relative coord: 
        [[ 0,  0,  0, -1, -1, -1, -2, -2, -2],
        [ 0,  0,  0, -1, -1, -1, -2, -2, -2],
        [ 0,  0,  0, -1, -1, -1, -2, -2, -2],
        [ 1,  1,  1,  0,  0,  0, -1, -1, -1],
        ...
        [ 2,  1,  0,  2,  1,  0,  2,  1,  0]]
        '''
        relative_coords = relative_coords.transpose([1, 2, 0]) # (9, 9, 2)
    ...
    def call(self, x, mask=None):
        ...
        # print("self.relative_position_bias_table:",self.relative_position_bias_table.shape) # TensorShape([25, 5]) 5 for 4 heads
        relative_position_bias = tf.gather(self.relative_position_bias_table, tf.reshape(
            self.relative_position_index, shape=[-1]))
        # print("relative_position_bias:",relative_position_bias.shape) # TensorShape([81, 5])
        relative_position_bias = tf.reshape(relative_position_bias, shape=[
                                            self.window_size[0] * self.window_size[1], \
                                            self.window_size[0] * self.window_size[1], -1])
        # print("relative_position_bias:",relative_position_bias.shape) # TensorShape([9, 9, 5])
        relative_position_bias = tf.transpose(
            relative_position_bias, perm=[2, 0, 1])
        # print("relative_position_bias:",relative_position_bias.shape) # TensorShape([5, 9, 9])
...
class SwinTransformerBlock(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0, mlp_ratio=4.,
                 qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path_prob=0., norm_layer=tf.keras.layers.LayerNormalization, prefix=''):
        super().__init__()
        self.dim = dim
    ...
    def build(self, input_shape):
        if self.shift_size > 0:
            ...
            h_slices = (slice(0, -self.window_size),
                        ...
                        slice(-self.shift_size, None)) # (slice(0, -3, None), slice(-3, -3, None), slice(-3, None, None))
            ...
            for h in h_slices:
                for w in w_slices:
                    ...
                    cnt += 1
            '''
            This is that the weird shift of image, which the masked places add a -100 to suppress the activation.
            (1, 12, 12, 1)
            (1, 12, 2, 1)
            (1, 12, 1, 1)
            (1, 2, 12, 1)
...
class PatchMerging(tf.keras.layers.Layer):
    def __init__(self, input_resolution, dim, norm_layer=tf.keras.layers.LayerNormalization, prefix=''):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = dim
...
class BasicLayer(tf.keras.layers.Layer):
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
                 ...
                 drop_path_prob=0., norm_layer=tf.keras.layers.LayerNormalization, downsample=None, use_checkpoint=False, prefix=''):
        super().__init__()
        self.dim = dim
        self.input_resolution = input_resolution
        self.depth = depth
